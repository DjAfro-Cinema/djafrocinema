...
type FormMode =
  | "default"        // normal email+password tabs
  | "forgot"         // forgot password → send reset link
  | "otp-email"      // enter email to receive OTP code
...
export default function AuthPage() {
  ...
  useEffect(() => {
    if (success) {
      playSuccessLottie();
      // Only auto-redirect on actual login success (not "code sent" message)
  ...
  // ── Sync OTP step: when userId arrives move to verify screen ───────────
  useEffect(() => {
      ...
      <style>{`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        :root {
          ...
          --font-display: var(--font-display); --font-body: 'DM Sans', sans-serif;
        ...
        .auth-visual-video, .auth-visual-img { width: 100%; height: 100%; object-fit: cover; object-position: center; display: block; }
        .auth-visual-img { transition: opacity 1.2s var(--ease-out); }
        ...
        .auth-visual-logo { position: absolute; top: 36px; left: 36px; z-index: 5; display: flex; align-items: center; gap: 10px; }
        .auth-logo-icon { width: 500px; height: 70px; border-radius: 4px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
        .auth-logo-text { font-family: var(--font-display); font-size: 1.35rem; letter-spacing: 0.12em; color: #fff; line-height: 1; }
        .auth-logo-text em { font-style: normal; color: var(--red); }
        .auth-visual-content { position: relative; z-index: 5; padding: 0 40px 48px; }
        ...
        .auth-visual-headline { font-family: var(--font-display); font-size: clamp(2.8rem, 4.5vw, 4.2rem); color: #fff; letter-spacing: 0.04em; line-height: 0.90; margin-bottom: 10px; transition: opacity 0.6s var(--ease-out), transform 0.6s var(--ease-out); }
        ...
        .auth-stat-n { font-family: var(--font-display); font-size: 1.6rem; color: #fff; letter-spacing: 0.06em; line-height: 1; }
        .auth-stat-l { font-size: 9px; letter-spacing: 0.3em; text-transform: uppercase; color: rgba(255,255,255,0.25); margin-top: 3px; }
        .auth-visual-pips { position: absolute; top: 50%; right: 20px; transform: translateY(-50%); z-index: 6; display: flex; flex-direction: column; gap: 6px; align-items: center; }
        ...
        .auth-form-panel { display: flex; flex-direction: column; justify-content: center; align-items: center; padding: 48px 40px; position: relative; background: var(--bg); overflow-y: auto; }
        ...
        .auth-mobile-logo { display: none; align-items: center; gap: 10px; margin-bottom: 36px; }
        ...
        .auth-heading-title { font-family: var(--font-display); font-size: clamp(2.4rem, 4vw, 3.2rem); color: #fff; letter-spacing: 0.05em; line-height: 0.88; margin-bottom: 8px; }
        .auth-heading-sub { font-family:'Outfit', system-ui, sans-serif !important; font-size: 12.5px; color: var(--text-muted); line-height: 1.5; }
        .auth-heading-sub strong { color: crimson; font-weight: 500; }
        /* OTP button — same style as old Google button */
        .auth-otp-btn { font-family:'Outfit', system-ui, sans-serif !important; display: flex; align-items: center; justify-content: center; gap: 12px; width: 100%; padding: 14px 20px; border-radius: 6px; border: 1px solid rgba(255,255,255,0.10); background: rgba(255,255,255,0.04); color: rgba(255,255,255,0.80); font-size: 13px; font-weight: 500; cursor: pointer; transition: background 0.2s, border-color 0.2s, color 0.2s; backdrop-filter: blur(8px); }
        ...
        .auth-input { width: 100%; padding: 13px 16px; border-radius: 6px; border: 1px solid var(--border); background: rgba(255,255,255,0.03); color: #fff; font-family: var(--font-body); font-size: 14px; outline: none; transition: border-color 0.22s, background 0.22s, box-shadow 0.22s; backdrop-filter: blur(6px); }
        ...
        /* OTP digit input — bigger and centered */
        ...
        .auth-form-inner { opacity: 0; transform: translateY(24px); transition: opacity 0.65s var(--ease-out) 0.15s, transform 0.65s var(--ease-out) 0.15s; }
        .auth-page.mounted .auth-form-inner { opacity: 1; transform: translateY(0); }
        .auth-visual-content { opacity: 0; transform: translateY(20px); transition: opacity 0.75s var(--ease-out) 0.1s, transform 0.75s var(--ease-out) 0.1s; }
        .auth-page.mounted .auth-visual-content { opacity: 1; transform: translateY(0); }
        .auth-visual-logo { opacity: 0; transition: opacity 0.5s ease 0.05s; }
        ...
        @media (max-width: 768px) {
          ...
          .auth-visual { height: 260px; flex-shrink: 0; }
          .auth-visual-pips { right: 12px; }
          .auth-visual-logo { top: 20px; left: 20px; }
          .auth-visual-content { padding: 0 24px 28px; }
          .auth-visual-headline { font-size: 2.2rem; }
          .auth-visual-sub { font-size: 12px; }
          .auth-stats { display: none; }
          .auth-visual-ov3 { display: none; }
          .auth-form-panel { padding: 36px 24px 48px; justify-content: flex-start; }
          .auth-mobile-logo { display: flex; }
          .auth-form-inner { max-width: 100%; }
        }
        @media (max-width: 420px) { .auth-form-panel { padding: 28px 18px 40px; } .auth-visual { height: 220px; } }
        .auth-img-stack { position: absolute; inset: 0; }
        .auth-img-layer { position: absolute; inset: 0; background-size: cover; background-position: center; transition: opacity 1.4s var(--ease-out); }
      `}</style>
      ...
      <div className={`auth-page${mounted ? " mounted" : ""}`}>
        ...
        <div className="auth-visual">
          <div className="auth-visual-media">
            {tab === "signup" ? (
              <video className="auth-visual-video" src={BG_VIDEO} autoPlay muted loop playsInline />
            ) : (
              <div className="auth-img-stack">
                {BG_IMAGES.map((src, i) => (
                  <div key={i} className="auth-img-layer" style={{ backgroundImage: `url(${src})`, opacity: i === bgIdx ? 1 : 0 }} />
                ))}
              </div>
            )}
          </div>
          ...
          <div className="auth-visual-logo">
            ...
            <Image src="/logo.png" alt="DjAfro Cinema" fill className="object-contain object-left" priority
              style={{ filter: "drop-shadow(0 0 12px rgba(229,9,20,0.6)) drop-shadow(0 0 24px rgba(229,9,20,0.3))", transition: "filter 0.4s ease" }}
            />
          </div>

          <div className="auth-visual-pips">
            {BG_IMAGES.map((_, i) => (
              <div key={i} className={`auth-pip${i === bgIdx ? " active" : ""}`} style={{ height: i === bgIdx ? 28 : 6 }} />
            ))}
          </div>

          <div className="auth-visual-content">
            <div className="auth-visual-tag"><span className="auth-visual-tag-dot" />DjAfro Cinema</div>
            <h2 className="auth-visual-headline">{tagline.line}<br /><em>{tagline.sub}</em></h2>
        ...
        <div className="auth-form-panel">
          ...
          <div className="auth-form-inner">
            ...
            <div className="auth-mobile-logo">
              ...
              <div className="auth-logo-text" style={{ color: "#fff" }}>DJ<span style={{ color: "#e50914" }}>AFRO</span></div>
            </div>
            ...
            {mode === "default" && (
              <>
                ...
                <div className="auth-tabs">
                  ...
                  <button className={`auth-tab${tab === "signup" ? " active" : ""}`} onClick={() => { setTab("signup"); clearMessages(); }}>Create Account</button>
                ...
                {/* Email OTP button — replaces Google */}
                <button className="auth-otp-btn" onClick={() => { clearMessages(); setMode("otp-email"); }} disabled={isLoading}>

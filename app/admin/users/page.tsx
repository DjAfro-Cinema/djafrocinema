...
export default function UsersPage() {
  ...
  const sessionUnknown = users.filter(u => !u.sessionType || u.sessionType === 'unknown').length;
  ...
  const filtered = users.filter(u => {
    ...
    const matchSession  = session === 'all' || (u.sessionType || 'unknown') === session;
  ...
  const sessionColor  = (s: string | undefined) => {
    if (s === 'pwa') return '#a855f7';
    if (s === 'app') return '#25d366';
    return '#6b7280';
  };
        ...
        {!loading && (
          <>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <ChartCard title="Signup Methods" height={120}>
              ...
              <ChartCard title="Platform" height={120}>
              ...
              <ChartCard title="Last session via" height={120}>
                <ChartCanvas deps={[sessionPWA, sessionApp, sessionUnknown]} config={{
                  type: 'doughnut',
                  data: { labels: ['PWA', 'App', 'Unknown'], datasets: [{ data: [sessionPWA, sessionApp, sessionUnknown], backgroundColor: ['#a855f7bb', '#25d366bb', '#6b728088'], borderColor: '#080810', borderWidth: 3, hoverOffset: 5 }] },
                  options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'right', labels: { color: 'rgba(255,255,255,0.5)', padding: 12, boxWidth: 8, font: { size: 10, family: 'DM Mono' } } } } },
                }} />
              </ChartCard>
            </div>
        ...
        <div style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 8, padding: '24px 28px' }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center', marginBottom: 20 }}>
            <input
              ...
              style={{ padding: '8px 14px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 4, color: '#fff', fontFamily: "'DM Mono',monospace", fontSize: 12, outline: 'none', width: 240 }}
            />
            ...
            {(['all', 'pwa', 'app', 'unknown'] as SessionFilter[]).map(s => (
              <button key={s} onClick={() => setSession(s)} style={{
                padding: '6px 14px', borderRadius: 3, cursor: 'pointer', fontFamily: "'DM Mono',monospace", fontSize: 10,
                background: session === s ? 'rgba(37,211,102,0.08)' : 'transparent',
                border: `1px solid ${session === s ? 'rgba(37,211,102,0.25)' : 'rgba(255,255,255,0.08)'}`,
                color: session === s ? '#25d366' : 'rgba(255,255,255,0.35)',
              }}>{s === 'all' ? 'All Sessions' : s === 'pwa' ? 'PWA Session' : s === 'app' ? 'App Session' : 'Unknown'}</button>
            ))}
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: "'DM Mono',monospace" }}>
              ...
              <tbody>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td style={{ padding: '12px 14px' }}>
                        <Badge label={u.platform === 'pwa' ? 'PWA' : 'Mobile'} color={platformColor(u.platform)} />
                      </td>
                      <td style={{ padding: '12px 14px' }}>
                        <Badge label={u.signupMethod} color={methodColor(u.signupMethod)} />
                      </td>
                      <td style={{ padding: '12px 14px' }}>
                        <Badge label={sessionLabel(u.sessionType)} color={sessionColor(u.sessionType)} />
                      </td>
                      <td style={{ padding: '12px 14px', fontSize: 12, color: 'rgba(255,255,255,0.5)', textAlign: 'center' }}>
                        {u.loginCount ?? '—'}
                      </td>
                      ...
                      <td style={{ padding: '12px 14px' }}>
                        <Badge label={u.isActive ? 'Active' : 'Inactive'} color={u.isActive ? '#22c55e' : '#6b7280'} />
                      </td>
                    </tr>
                  );
                })}
                {filtered.length === 0 && (
                  <tr>
                    <td colSpan={8} style={{ padding: '40px', textAlign: 'center', color: 'rgba(255,255,255,0.2)', fontSize: 13 }}>

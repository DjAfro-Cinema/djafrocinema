...
import {
  ...
  Subtitles, Rewind, FastForward, Film,
} from "lucide-react";
...
function StarRating({
  ...
  // Persist to localStorage so rating survives page refresh
  useEffect(() => {
  ...
  const handleRate = async (stars: number) => {
    ...
    try {
      ...
      // Simple weighted average — store new rating as blend
        ...
        {Array.from({ length: 5 }).map((_, i) => {
          ...
          return (
            <button
              ...
              style={{
                width: 36, height: 36,
                background: filled ? "rgba(229,9,20,0.12)" : "rgba(255,255,255,0.04)",
                border: `1px solid ${filled ? "rgba(229,9,20,0.35)" : "rgba(255,255,255,0.08)"}`,
            ...
            >
              <Star
                size={16}
                fill={filled ? "#e50914" : "transparent"}
                color={filled ? "#e50914" : "rgba(255,255,255,0.2)"}
              />
            </button>
          );
        })}
...
function VideoPlayer({ movie, onClose }: { movie: Movie; onClose: () => void }) {
  ...
  const totalSec = 7200; // default 2h — actual video handled by <video> tag if video_url exists
  ...
  // If there's a real video_url, use an actual <video> element
  if (movie.video_url) {
    return (
              ...
              <div style={{ position: "relative", width: "100%", background: "#000" }}>
                {playerOpen ? (
                  ...
                  <div style={{ position: "relative", width: "100%", aspectRatio: "16/9", overflow: "hidden" }}>
                    ...
                    {/* Center play */}
                    <button onClick={() => setPlayerOpen(true)} style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", width: 80, height: 80, background: "rgba(229,9,20,0.9)", border: "none", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", boxShadow: "0 0 60px rgba(229,9,20,0.5)" }}>
              ...
              <div style={{
                ...
                <div>
                  ...
                  <div style={{ display: "flex", gap: 10, marginBottom: 36, flexWrap: "wrap" }}>
                    <button onClick={() => setPlayerOpen(true)} style={{ display: "flex", alignItems: "center", gap: 10, padding: "13px 30px", background: "#e50914", border: "none", borderRadius: 8, color: "#fff", fontSize: 13, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", fontFamily: "'DM Sans', sans-serif", cursor: "pointer", boxShadow: "0 4px 20px rgba(229,9,20,0.35)" }}>
                    ...
                    <div>
                      ...
                      <div style={{ padding: "24px 20px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 12 }}>
                        <StarRating
                          ...
                          onRated={r => setLiveRating(r)}
                        />
                      </div>
                    </div>
                  )}

                  {/* Tab: Cast (tags-based since Appwrite doesn't have cast field) */}
                  {activeTab === "cast" && (
